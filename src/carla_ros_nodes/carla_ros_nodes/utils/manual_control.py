...
try:
    ...
    from pygame.locals import (K_0, K_9, K_BACKQUOTE, K_BACKSPACE, K_COMMA,
                               ...
                               K_c, K_d, K_f, K_g, K_h, K_i, K_l, K_m, K_n,
                               K_o, K_p, K_q, K_r, K_s, K_t, K_v, K_w, K_x,
                               K_z)
except ImportError:
...
class World(object):
    ...
    def restart(self):
        ...
        if vehicles:
            self.player = vehicles[0]
            print(f"Attached to vehicle: {self.player.type_id}, ID: {self.player.id}")
            self.show_vehicle_telemetry = False
            self.modify_vehicle_physics(self.player)
        else:
